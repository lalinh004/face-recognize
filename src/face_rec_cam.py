...
def main():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Tạo một dictionary để lưu trữ embeddings của các khuôn mặt đã biết
            known_embeddings = {}

            # Tạo embeddings cho từng khuôn mặt đã biết
            for i, class_name in enumerate(class_names):
                class_images = []  # Thêm ảnh của lớp (người) vào đây
                embeddings_list = []
                ...
                if faces_found > 0:
                    ...
                    for i in range(faces_found):
                        ...
                        if (bb[i][3] - bb[i][1]) / frame.shape[0] > 0.25:
                            ...
                            if cropped.size > 0:  # Thêm kiểm tra kích thước cropped
                                ...
                                if best_class_probabilities[0] > 0.7:
                                    ...
                                    if min_dist < 1.0:  # Ngưỡng khoảng cách Euclidean
                                        cv2.rectangle(frame, (bb[i][0], bb[i][1]), (bb[i][2], bb[i][3]), (0, 255, 0), 2)
                                        text_x = bb[i][0]
                                        text_y = bb[i][3] + 20
                                        cv2.putText(frame, closest_name, (text_x, text_y), cv2.FONT_HERSHEY_COMPLEX_SMALL,
                                                    1, (255, 255, 255), thickness=1, lineType=2)
                                        cv2.putText(frame, str(round(min_dist, 3)), (text_x, text_y + 17),
                                                    cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 255), thickness=1, lineType=2)
                                    else:
